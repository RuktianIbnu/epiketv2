...
import (
	ku "epiketv2/http/usecase/kegiatan"
...
func (m *handler) Create(c *gin.Context) {
	...
	lastID, err := m.kegiatanUsecase.Create(&data)
	if err != nil {
...
func (m *handler) UpdateOneByID(c *gin.Context) {
	...
	_, err := m.kegiatanUsecase.UpdateOneByID(&data)
	if err != nil {
		c.JSON(resp.Format(http.StatusInternalServerError, err))
		return
	}
...
func (m *handler) GetOneByID(c *gin.Context) {
	...
	data, err := m.kegiatanUsecase.GetOneByID(ids)
	if err != nil {
		c.JSON(resp.Format(http.StatusInternalServerError, err))
		return
	}
...
func (m *handler) GetAll(c *gin.Context) {
	...
	list, totalEntries, err := m.kegiatanUsecase.GetAll(dqp)
	if err != nil {
		c.JSON(resp.Format(http.StatusInternalServerError, err))
		return
	}
...
func (m *handler) DeleteOneByID(c *gin.Context) {
	...
	_, err := m.kegiatanUsecase.DeleteOneByID(ids)
	if err != nil {
		c.JSON(resp.Format(http.StatusInternalServerError, err))
		return
	}
