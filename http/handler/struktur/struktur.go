...
import (
	su "epiketv2/http/usecase/struktur"
...
func (m *handler) Create(c *gin.Context) {
	...
	lastID, err := m.strukturUsecase.Create(&data)
	if err != nil {
...
func (m *handler) UpdateOneByID(c *gin.Context) {
	...
	_, err := m.strukturUsecase.UpdateOneByID(&data)
	if err != nil {
		c.JSON(resp.Format(http.StatusInternalServerError, err))
		return
	}
...
func (m *handler) GetOneByID(c *gin.Context) {
	...
	data, err := m.strukturUsecase.GetOneByID(ids)
	if err != nil {
		c.JSON(resp.Format(http.StatusInternalServerError, err))
		return
	}
...
func (m *handler) GetAll(c *gin.Context) {
	...
	list, totalEntries, err := m.strukturUsecase.GetAll(dqp)
	if err != nil {
		c.JSON(resp.Format(http.StatusInternalServerError, err))
		return
	}
...
func (m *handler) DeleteOneByID(c *gin.Context) {
	...
	_, err := m.strukturUsecase.DeleteOneByID(ids)
	if err != nil {
		c.JSON(resp.Format(http.StatusInternalServerError, err))
		return
	}
