...
import (
	thu "epiketv2/http/usecase/tx_harian"
...
func (m *handler) Create(c *gin.Context) {
	...
	lastID, err := m.tx_hairanUsecase.Create(&data)
	if err != nil {
...
func (m *handler) UpdateOneByID(c *gin.Context) {
	...
	_, err := m.tx_hairanUsecase.UpdateOneByID(&data)
	if err != nil {
		c.JSON(resp.Format(http.StatusInternalServerError, err))
		return
	}
...
func (m *handler) GetOneByID(c *gin.Context) {
	...
	data, err := m.tx_hairanUsecase.GetOneByID(ids)
	if err != nil {
		c.JSON(resp.Format(http.StatusInternalServerError, err))
		return
	}
...
func (m *handler) GetAll(c *gin.Context) {
	...
	list, totalEntries, err := m.tx_hairanUsecase.GetAll(dqp)
	if err != nil {
		c.JSON(resp.Format(http.StatusInternalServerError, err))
		return
	}
...
func (m *handler) DeleteOneByID(c *gin.Context) {
	...
	_, err := m.tx_hairanUsecase.DeleteOneByID(ids)
	if err != nil {
		c.JSON(resp.Format(http.StatusInternalServerError, err))
		return
	}
